 ...
 * outbox owns durability, backoff and dead-lettering. What remains is to describe
 * the material, hand it over, and write down what came back.
 *
...
export async function deliverReportOutboxEvent(event: ModerationOutboxEvent): Promise<void> {
  ...
  if (!report) {
     ...
     */
    log.moderation.warn({ reportId }, '[CrowdSource] delivery event has no report');
    return;
  }
   ...
   * human. Catching it and writing a local state would put the report somewhere
   * nothing alerts on.
   */
