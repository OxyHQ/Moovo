 ...
 * produce them:
 *
 * - `violation` → `resolved`: the case reached a conclusion Moovo can act on.
...
import type { ModerationLocalStatus, ReportStatus } from '@moovo/shared-types';
 ...
 * outcome must be handled rather than throw. The known values are matched
...
/**
 * Decision statuses that end Moovo's side of the case.
 *
 * A `provisional` decision leaves the report at `submitted`: a later revision may
 * supersede it, and a report Moovo had already closed would have to be reopened.
 * `superseded` is not here either — a superseded revision is not the current
 * answer and must never be the one that closes the report.
 */
